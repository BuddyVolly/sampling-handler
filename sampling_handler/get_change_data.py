...
from .helpers.ee_helpers import processing_grid
...
def upload_tmp_asset(asset_root, fc, asset_name, create_folder=True):
    ...
    try:
        ...
        child_assets = ee.data.listAssets({"parent": f"{asset_root}/tmp_sbae"})[
            "assets"
        ]
...
def upload_missing_points(df, point_id_name, fc, asset_name):
    ...
    if df is not None:
        ...
        iterative_fc = fc.filter(
            ee.Filter.inList(point_id_name, ee.List(processed_points)).Not()
        )
...
def aggregate_tmp_files(tmpdir):
    ...
    for file in tmp_files:
        ...
        df = (
            pd.concat([df, pd.read_pickle(file)], ignore_index=True)
            if df is not None
            else pd.read_pickle(file)
        )
        file.unlink()
...
def extract_to_df(sat_coll, cell_fc, config_file):
    ...
    if bfast or cusum or ts_metrics or bs_slope or ccdc or landtrendr or jrc_nrt:
        ...
        if ccdc:
            ...
            check_bpb = all(
                item in bands for item in config_dict["ccdc_params"]["breakpointBands"]
            )
            if "tmaskBands" in config_dict["ccdc_params"]:
                check_tmask = all(
                    item in bands for item in config_dict["ccdc_params"]["tmaskBands"]
                )
            else:
                check_tmask = True
        ...
        df[["dates_mon", "ts_mon", "mon_images"]] = df.apply(
            lambda row: subset_ts(
                row, config_dict["ts_params"]["start_monitor"], bands
            ),
            axis=1,
            result_type="expand",
        )
...
def get_change_data(fc, config_dict):
    ...
    iterative_fc, left_to_process = upload_missing_points(
        df, point_id_name, fc, "tmp_initial_fc"
    )
    ...
    for grid_size in grid_sizes:
        ...
        if left_to_process > 0:
            ...
            param_string = (
                f"{sat}_{ts_band}_{start_hist}_{start_mon}_{end_mon}_{grid_size}"
            )
            ...
            lsat = landsat_collection(
                start_hist, end_mon, aoi, **config_dict["lsat_params"]
            )
            ...
            print(
                f" Splitting the aoi in chunks for parallel processing (Level {grid_sizes.index(grid_size)+1})."
            )
            print(
                f" Parallelizing on chunks of {grid_size}x{grid_size} degrees, totalling in {len(grid)} chunks."
            )
            ...
            def cell_computation(args):
                ...
                tmp_empty_file = tmpdir.joinpath(
                    f"tmp_noresults_{idx}_{param_string}.txt"
                )

                # check if already been calculated
                if tmp_file.exists() or tmp_empty_file.exists():
                    print(
                        f" Chunk {idx+1} at chunksize of {grid_size} degrees already has been extracted. Going on with next chunk."
                    )
                    return
                ...
                if nr_of_points > 0 and nr_of_points < max_points_per_chunk:
                    ...
                    print(
                        f" Chunk {idx+1} with {nr_of_points} points done in: {timedelta(seconds=elapsed)}"
                    )
                    ...
                    with open(tmp_empty_file, "w") as f:
                        f.write("0 points")
                    print(
                        f" Chunk {idx+1} does not contain any points. Going on with next chunk."
                    )
                    ...
                    print(
                        f" More than {max_points_per_chunk} points in chunk {idx+1}. Considering respective points at smaller chunk size level."
                    )
            ...
            executor = Executor(
                executor="concurrent_threads", max_workers=config_dict["workers"]
            )
            for i, task in enumerate(
                executor.as_completed(func=cell_computation, iterable=args_list)
            ):
                try:
                    task.result()
                except:
                    print(
                        " Gridcell task failed. Trying to process the respective points at a lower chunk size."
                    )
                    pass
        ...
        if df is not None:
            ...
            if len(df) < nr_total_points:
                ...
                iterative_fc, left_to_process = upload_missing_points(
                    df, point_id_name, fc, asset_name
                )
    ...
    if "dates" in df.columns:
        gdf = gpd.GeoDataFrame(
            df.drop(["dates", "ts"], axis=1), crs="EPSG:4326", geometry=df["geometry"]
        )
    else:
