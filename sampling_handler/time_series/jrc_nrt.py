...
def get_magnitudes(da, config_dict):
    ...
    try:
        ...
        CuSumMonitor.fit(
            dataarray=history, trend=False, method="ROC", screen_outliers="Shewhart"
        )
        ...
        df = pd.DataFrame(
            {
                ...
                "mosum_jrc_change": np.where(
                    MoSumMonitor.detection_date[mask] > 0, 1, 0
                ),
                "mosum_jrc_magnitude": MoSumMonitor.process[mask],
                "cusum_jrc_date": CuSumMonitor.detection_date[mask],
                "cusum_jrc_change": np.where(
                    CuSumMonitor.detection_date[mask] > 0, 1, 0
                ),
...
def run_jrc_nrt(df, config_dict):
    ...
    new_df = pd.DataFrame(columns=["time", "x", "y", "data", point_id_name]).set_index(
        ["time", "x", "y"]
    )
    ...
    for i, row in df.iterrows():
        ...
        arrays = [row.dates, [x for i in range(len(ts))], [y for i in range(len(ts))]]
        ...
        new_df = pd.concat(
            [
                new_df,
                pd.DataFrame(
                    {"data": ts, point_id_name: point_ids}, index=arrays
