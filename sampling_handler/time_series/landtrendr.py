...
def run_landtrendr(df, points, config_dict):
    ...
    for i, row in df.iterrows():
        ...
        # get years of ts
        years = np.unique(
            [date.year for date in row.dates if date.year >= int(start_monitor)]
        )
    ...
    # run lndtrendr
    lt = ee.Algorithms.TemporalSegmentation.LandTrendr(**landtrendr_params).select(
        ["LandTrendr"]
    )
    ...
    sortByThis = segInfo.arraySlice(0, 4, 5).toArray(0).multiply(-1)
    # need to flip the delta here, since arraySort is working by ascending order
    segInfoSorted = segInfo.arraySort(sortByThis)
    # sort the array by magnitude
    bigDelta = segInfoSorted.arraySlice(1, 0, 1)
    # get the first segment in the sorted array (greatest magnitude vegetation loss segment)

    bigDeltaImg = ee.Image.cat(
        bigDelta.arraySlice(0, 0, 1).arrayProject([1]).arrayFlatten([["yod"]]),
        bigDelta.arraySlice(0, 1, 2).arrayProject([1]).arrayFlatten([["endYr"]]),
        bigDelta.arraySlice(0, 2, 3)
        .arrayProject([1])
        .arrayFlatten([["startVal"]])
        .multiply(distDir),
        bigDelta.arraySlice(0, 3, 4)
        .arrayProject([1])
        .arrayFlatten([["endVal"]])
        .multiply(distDir),
        bigDelta.arraySlice(0, 4, 5)
        .arrayProject([1])
        .arrayFlatten([["mag"]])
        .multiply(distDir),
        bigDelta.arraySlice(0, 5, 6).arrayProject([1]).arrayFlatten([["dur"]]),
        bigDelta.arraySlice(0, 6, 7)
        .arrayProject([1])
        .arrayFlatten([["rate"]])
        .multiply(distDir),
    )
    ...
    distMask = (
        bigDeltaImg.select(["mag"]).lt(1000).And(bigDeltaImg.select(["dur"]).lt(5))
    )
    ...
    def pixel_value_nan(feature):
        pixel_value = ee.List([feature.get(ts_band), -9999]).reduce(
            ee.Reducer.firstNonNull()
        )
        return feature.set({ts_band: pixel_value})

    sampled_points = (
        bigFastDist.select(["mag", "dur", "yod", "rate", "endYr"])
        .reduceRegions(
            **{
