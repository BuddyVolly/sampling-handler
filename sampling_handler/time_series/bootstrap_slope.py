...
def bootstrap_slope(args):
    ...
    if x:
        ...
        for _ in range(nr_bootstraps):
            ...
            rand_idx = sorted(
                np.random.choice(np.arange(y.size), int(y.size * size), replace=False)
            )
...
def run_bs_slope(df, config_dict):
    ...
    for i, row in df.iterrows():
        ...
        args_list.append(
            [row.ts_mon[ts_band], dates_float, nr_of_bootstraps, row[point_id_name]]
        )

    executor = Executor(executor="concurrent_threads", max_workers=16)
    for i, task in enumerate(
        executor.as_completed(func=bootstrap_slope, iterable=args_list)
    ):
        try:
