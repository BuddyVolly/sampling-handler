...
def cusum_deforest(args):
    ...
    if data:
        ...
        mask = tf.convert_to_tensor(
            np.isfinite(data).astype("float32"), dtype="float32"
        )

        # calculate mean
        mean_tf = tf.math.divide_no_nan(
            tf.math.reduce_sum(stack_tf, axis=0), tf.math.reduce_sum(mask, axis=0)
        )
...
def run_cusum_deforest(df, config_dict):
    ...
    for i, row in df.iterrows():
        dates_float = [
            date.year + np.round(date.dayofyear / 365, 3) for date in row.dates_mon
        ]
        args_list.append(
            [row.ts_mon[ts_band], dates_float, row[point_id_name], nr_of_bootstraps]
        )

    executor = Executor(executor="concurrent_threads", max_workers=16)
    for i, task in enumerate(
        executor.as_completed(func=cusum_deforest, iterable=args_list)
    ):
        try:
